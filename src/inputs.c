...
/*
 * 				SET LED STATE
 * turns LED on when State = 1 and off when State = 0
 */
...
/*
 * 				OUTPUT ON/OFF BUTTON INITIALIZATION
 */
void ButtonInit(void) {
	...
	NVIC_InitStruct.NVIC_IRQChannelPreemptionPriority = OUTPUT_BUTTON_PREEMPTION;
	NVIC_InitStruct.NVIC_IRQChannelSubPriority = OUTPUT_BUTTON_SUB;
	NVIC_InitStruct.NVIC_IRQChannelCmd = ENABLE;
	NVIC_Init(&NVIC_InitStruct);
}

/*
 * 				OUTPUT ON/OFF BUTTON INTERRUPT HANDLER
 * increments button release counter when button is released
 */
...
/*
 * 				OUTPUT ON/OFF BUTTON RELEASE
 * returns number of output button releases since this function was last called
 */
uint8_t ButtonRelease(void){

	uint8_t releases = output_button;
	output_button = 0;
	return releases;
}
